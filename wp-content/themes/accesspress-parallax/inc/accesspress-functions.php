 ...
 */
if( ! function_exists('accesspress_parallax_excerpt_content')):
    function accesspress_parallax_excerpt_content($count){
    if( empty($count)){
        return;
    }
    ...
    if($count){
        $excerpt   = '<div class="expert-content"><p>'.$excerpt.'</p></div>'.'<a href="'.esc_url($permalink).'" class="read-more">'.esc_html__("Continue Reading","accesspress-parallax").'<span>&rarr;</span>'.'</a>';
    }
    return $excerpt;
    }
endif;